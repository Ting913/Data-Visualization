...
ci.lodiboxplot95 <- function(measured.values,lod,n.lod,lambda=0,right.quantile=0.75,conf.level=0.95,n.bootstrap=1000){  #n.bootstrap =10 only for test, should be 1000
    #cat(paste0("Current lambda value is: ", lambda, "\n"))
                    ...
                    tabPanel("Tutorial", 
                             ...
                             fluidRow(
                                 column(12, align = "justify",
                                        tags$p(strong("Here is the user manual for this program. It contains information on how to use the program and what to expect when using it."), style = "font-size: 20px;"))
                             ),
                             h2(strong("Usage Flow")),
                             fluidRow(
                                 column(12, align = "justify",
                                        tags$img(src="images/image.png", width = "50%", height = "auto"))
                             ),
                             #tags$img(src = file.path(getwd(), "www", "images", "image.png")),
                             #img(src = "./images/image.png", alt = "Usage Flow Image", width = "100%", height = "auto"),
                             h2(strong("Important Notes")),
                             tags$ul(
                                 tags$li(tags$b("File:", style = "font-size: 20px;"), " Please upload only comma-separated CSV files.", style = "font-size: 20px;"),
                                 tags$li(tags$b("Data:", style = "font-size: 20px;"), 
                                         tags$ul(
                                             tags$li("The first LOD value is only to identify the LOD value, which can be a number, a string, or a na value. Note that if it is a na value, please use lowercase.", style = "font-size: 20px;"),
                                             tags$li("The second LOD value is the real limit of detection (LOD), which can only be entered as a number.", style = "font-size: 20px;"),
                                             tags$li("Selecting filter columns and display columns means: for example, if you need to calculate the TnT values for all males aged 20-30, the filter columns are age and sex, and the display column is TnT.", style = "font-size: 20px;")
                                         )),
                                 tags$li(tags$b("Diagrams:", style = "font-size: 20px;"), 
                                         tags$ul(
                                             tags$li("If the lambda value is not entered, it defaults to zero.", style = "font-size: 20px;"),
                                             tags$li("Please wait for a while when calculating the Confidence Level, as the calculation is time-consuming due to the large amount of data.", style = "font-size: 20px;")
                                         ))
                             )
                    ), #tutorial
                    
                    tabPanel("LODibosplot 95",
                    sidebarLayout(
                        sidebarPanel(
                            tags$h2("File:"),
                            fileInput("file", "Please upload comma-separated csv files"),
                            tags$h2("Data:"),
                            textInput("lod", "Enter LOD value in the table:", value = "",placeholder = "Please enter a number, string or na"),
                            numericInput("real_lod", "Please enter real detection limit to calculate (LOD):", value = 0, step = 0.1),
                            ...
                            actionButton("conf_level", "Confidence interval")
                        ...
                        mainPanel(
                            ...
                            h4("Output"),
                            actionButton("show_results_btn", "show results"),
                            conditionalPanel(
                                condition = "show_results()",
...
server <- function(input, output, session) {
    ...
    output$real_lod <- renderText({
        paste("The real-LOD value entered is:", input$real_lod)
    })
    
    data <- reactive({
        req(input$file)
        read.csv(input$file$datapath, header = TRUE, na.strings = c("", "NA"))
    })
    ...
    #output$table <- renderTable({
        ...
        for (filter_col in input$filter_cols) {
            if (filter_col %in% c("gender", "sex","Sex","geschlecht", "SEX")) {
        ...
        if (!is.null(x)) {
            selected_col_name <- selected_col()
            if (is.character(x[[selected_col_name]])) {
                if (is.na(input$lod)) {
                    num_string <- sum(is.na(x[[selected_col_name]]))
                } else {
                    if (is.numeric(input$lod)) {
                        num_string <- sum(as.character(x[[selected_col_name]]) == as.character(input$lod))
                    } else {
                        num_string <- sum(x[[selected_col_name]] == input$lod)
                    }
                }
            } else {
                if (is.na(input$lod)) {
                    num_string <- sum(is.na(x[which(is.na(x) | x == input$lod & names(x) == selected_col()),]))
                } else {
                    if (is.numeric(input$lod)) {
                        num_string <- sum(x[which(x == input$lod & names(x) == selected_col()),])
                    } else {
                        num_string <- sum(x[[selected_col_name]] == input$lod)
                    }
                }
            }
            x <- x[!is.na(x) & !(is.character(x) & names(x) == selected_col() )& x != 0, ]
        }
        ...
        if (!is.numeric(measured.values.m)) {
            #print("non-numeric")
            measured.values.m <- as.numeric(measured.values.m)  # Convert to numeric
        }
